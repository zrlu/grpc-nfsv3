//
// Provided starter code for CS 454
// This code implements a Reader-Writer lock for CS 454.
...
int rw_lock_lock(rw_lock_t* lock, rw_lock_mode_t mode) {
  ...
  if (mode == RW_READ_LOCK) {
    while ((lock->num_writers_ > 0) || (lock->num_waiting_writers_ > 0)) {
      // We can ignore this return call, because we are supposed to hold the
      // lock, and we know that the args are not invalid at this point.
...
int rw_lock_unlock(rw_lock_t* lock, rw_lock_mode_t mode) {
  ...
  // Signal waiters. There is nothing that we can do if there is an error here.
