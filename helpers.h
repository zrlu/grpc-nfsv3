...
void copytimespec2TimeSpec(struct timespec *src, TimeSpec *dst)
{
    dst->set_tv_sec(src->tv_sec);
    dst->set_tv_nsec(src->tv_nsec);
}

void copyTimeSpec2timespec(TimeSpec *src, struct timespec *dst)
{
    dst->tv_sec = src->tv_sec();
    dst->tv_nsec = src->tv_nsec();
}

void copystat2Stat(struct stat *src, Stat *dst)
{
...
void copyStat2stat(Stat *src, struct stat *dst)
{
    ...
    TimeSpec st_atime_ = src->st_atime_();
    TimeSpec st_mtime_ = src->st_mtime_();
    TimeSpec st_ctime_ = src->st_ctime_();
